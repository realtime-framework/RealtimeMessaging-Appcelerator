...
module_defaults = {
	...
	'copyright' : 'Copyright (c) %s by Realtime.co' % str(date.today().year),
}
...
def compile_js(manifest,config):
	js_file = os.path.join(cwd,'assets','co.realtime.apns.js')
	...
	f = os.path.join(cwd,'Classes','CoRealtimeOrtcModuleAssets.m')
	c = open(f).read()
	idx = c.find('return ')
	before = c[0:idx]
	after = """
...
def package_module(manifest,mf,config):
	...
	zf.write('co.realtime.ortc.apns.js', '%s/co.realtime.ortc.apns.js' % modulepath) 
	zf.close()
